...
# Load data
df <- read_csv("../FinalData/AllData.csv")
...
basic_effect_covar <- c("Sex", "education", "NESB_Status",  "living","moves","pp_dn_n10",
                     "ses_av_n10","hearaid","z_eTIV","Scanner_Type")
...
exposure_avg_var <- paste0(exposure_var, "_avg")
  ...
  write_csv(df,"../FinalData/AllData_Modelled.csv")
  ...
  for (region in hippocampal_regions) {
      ...
      formula_text1 <- paste0(region, "~ agec+ agec2+",
                             paste(basic_effect_covar, collapse = " + "),
                             " + (1+agec|MAS_ID)")
      lmer_output1<-run_lmer_models(df,  formula_text1,region, exposure_var) 
      estimates_table_basic<-bind_rows(estimates_table_basic,lmer_output1$estimates)
  ...
  estimates_wide_pivot1<-reshape_table(estimates_table_basic)
  selected_colnames <- names(estimates_wide_pivot1)[grepl("agec", names(estimates_wide_pivot1))]
  selected_estimates_wide_pivot1 <- estimates_wide_pivot1[, c("Region",selected_colnames)]
  selected_estimates_wide_pivot1$agec_p_value_adj<-p.adjust(selected_estimates_wide_pivot1$agec_p_value_fmt,method = "fdr")
  selected_estimates_wide_pivot1$agec2_p_value_adj<-p.adjust(selected_estimates_wide_pivot1$agec2_p_value_fmt,method = "fdr")
  
  
  df1_sel <- selected_estimates_wide_pivot1 %>%
    select(Region,
           Linear_CI = agec_Estimate_CI,
           Linear_t_value=agec_t_value,
           Linear_p_value = agec_p_value_adj,
           Nonlinear_CI = agec2_Estimate_CI,
           Nonlinear_t_value=agec2_t_value,
           Nonlinear_p_value = agec2_p_value_adj
           )
  
  write_csv(df1_sel, paste0("../FinalData/Table1_lmer_model_basic",region_type,'_',exposure_var, ".csv"))
  ...
  selected_estimates_wide_pivot5$si_dn_n10_avg_p_value_adj<-p.adjust(selected_estimates_wide_pivot5[["si_dn_n10_avg_p_value_fmt"]],method = "fdr")
  selected_estimates_wide_pivot5$agec_p_value_adj<-p.adjust(selected_estimates_wide_pivot5[["agec:si_dn_n10_avg_p_value_fmt"]],method = "fdr")
  selected_estimates_wide_pivot5$agec2_p_value_adj<-p.adjust(selected_estimates_wide_pivot5[["agec2:si_dn_n10_avg_p_value_fmt"]],method = "fdr")
  
  df2_sel <- selected_estimates_wide_pivot5 %>%
    select(Region,
           main_effect_CI = si_dn_n10_avg_Estimate_CI,
           main_effect_t_value=si_dn_n10_avg_t_value,
           main_effect_p_value = si_dn_n10_avg_p_value_adj,
           linear_CI = `agec:si_dn_n10_avg_Estimate_CI`,
           linear_t_value=`agec:si_dn_n10_avg_t_value`,
           linear_p_value = agec_p_value_adj,
           nonlinear_CI = `agec2:si_dn_n10_avg_Estimate_CI`,
           nonlinear_t_value=`agec2:si_dn_n10_avg_t_value`,
           nonlinear_p_value = agec2_p_value_adj
    )
  write_csv(df2_sel, paste0("../FinalData/Table2_lmer_model_direct",region_type,'_', exposure_var, ".csv"))
  ...
  df3_sel <- selected_estimates_wide_pivot5  %>%
    select(Region,
           Estimate_CI = si_dn_n10_centered_Estimate_CI,
           t_value=si_dn_n10_centered_t_value,
           p_value = si_dn_n10_centered_p_value_fmt)
  
  write_csv(df3_sel, "../FinalData/Supplementary_Table1_si_dn_n10_centered.csv")
  
  df4_sel <- selected_estimates_wide_pivot5  %>%
    select(Region,
           Estimate_CI = `agec:si_dn_n10_centered_Estimate_CI`,
           Estimate_t_value=`agec:si_dn_n10_centered_t_value`,
           Estimate_p_value = `agec:si_dn_n10_centered_p_value_fmt`)
  
  write_csv(df4_sel, "../FinalData/Supplementary_Table2_agec_si_dn_n10_centered.csv")
  
  
  df5_sel <- selected_estimates_wide_pivot5 %>%
    select(Region,
           Estimate_CI=`agec2:si_dn_n10_avg_Estimate_CI`,
           t_value=`agec2:si_dn_n10_avg_t_value`,
           p_value = `agec2:si_dn_n10_avg_p_value_fmt`)
  write_csv(df4_sel, "../FinalData/Supplementary_Table3_agec_si_dn_n10_centered.csv")
